...
export default function NBackDifficultyScreen({ navigation }: any) {
  const { theme } = useAppTheme();
  const styles = makeStyles(theme);

  function go(tier: 1 | 2 | 3 | 4) { navigation.navigate("NBackBlocks", { tier }); }
  ...
  return (
    <SafeAreaView style={styles.wrap}>
      ...
      <View style={styles.card}>
        ...
        <Pressable onPress={() => go(1)} style={styles.option}><Text style={styles.optTitle}>Beginner</Text><Text style={styles.optSub}>1-Back: same as the one before?</Text></Pressable>
        <Pressable onPress={() => go(2)} style={styles.option}><Text style={styles.optTitle}>Intermediate</Text><Text style={styles.optSub}>2-Back: same as 2 letters ago?</Text></Pressable>
        <Pressable onPress={() => go(3)} style={styles.option}><Text style={styles.optTitle}>Advanced</Text><Text style={styles.optSub}>2-Back Position: same spot as 2 ago?</Text></Pressable>
        <Pressable onPress={() => go(4)} style={styles.option}><Text style={styles.optTitle}>Expert</Text><Text style={styles.optSub}>3-Back: same as 3 letters ago?</Text></Pressable>
      </View>
      <View style={{ height: spacing.lg }} />
...
const makeStyles = (theme: any) => StyleSheet.create({
  ...
  card: { backgroundColor: "rgba(209, 225, 225, 0.5)", borderRadius: 20, padding: spacing.lg, shadowColor: "#000", shadowOpacity: 0.06, shadowRadius: 18, shadowOffset: { width: 0, height: 10 }, elevation: 2, marginTop: spacing.sm, gap: spacing.md },
  label: { color: theme.text, fontSize: 12, fontWeight: "800" },
  option: { borderRadius: 18, paddingVertical: spacing.md, paddingHorizontal: spacing.lg, backgroundColor: "rgba(255,255,255,0.55)", borderWidth: 1, borderColor: "rgba(0,0,0,0.06)", gap: 4 },
